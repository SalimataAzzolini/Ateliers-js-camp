...
function ajoutproduit() {
  ...
  stockArray.forEach(function (element) {
    //créer un element tr et donner un innerhtml + tr
...
function editProduit(trLine) {
  ...
  editButtonQuery.forEach(function (editDeletes, index) {
    editDeletes.addEventListener("click", function () {
      ...
      /*
      let nouveauProduit;
      let nouveauButtonQuery = document.querySelectorAll(".validerButton");
      nouveauButtonQuery.forEach(function (nouveauDeletes, index) {
        nouveauDeletes.addEventListener("click", function () {});
      });
      */
    });
  });
